//! DDSL コンパイラの WebAssembly バインディング。
//!
//! ブラウザ上で解析・診断・DDL 生成・シンタックスハイライトを行う。

use ddsl_core::span::{LineIndex, Span};
use ddsl_core::{Severity, codegen, dialect, highlight, parse, resolve};
use serde::Serialize;
...
/// DDSL のソースを色付けした HTML にする。CLI やサイトと同じ字句解析を使う。
#[wasm_bindgen]
...
fn to_diagnostic(index: &LineIndex, d: &ddsl_core::Diagnostic) -> Diagnostic {
